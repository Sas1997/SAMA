...
def calcTouRate(year, onPrice, midPrice, offPrice, onHours, midHours, season, daysInMonth, holidays, treat_special_days_as_offpeak):
  ...
  for m in range(12):
    ...
    for d in range(daysInMonth[m]):
      if tp is not None:
        for hour in tp:
          idx0 = t_index[hour] + 24 * d
          Cbuy[idx0] = P_peak

      if tm is not None:
        for hour in tm:
          idx1 = t_index[hour] + 24 * d
          Cbuy[idx1] = P_mid
