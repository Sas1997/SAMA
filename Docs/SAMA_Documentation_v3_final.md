# SAMA
## Solar Alone Multi-Objective Advisor V2

**Comprehensive User Documentation | v1**
...
- [4. Method 2 – Using the Python Package (pip install sama)](#4-method-2--using-the-python-package-pip-install-sama)
...
- [16. License and Citation](#16-license-and-citation)
  ...
  - [16.3 Citing SAMA](#163-citing-sama)
...
| **Method 2:** Python Package (pip) | Users who want to run SAMA as a command-line tool or use it in their own Python scripts without modifying internals | `pip install sama` (available as the SAMA Py Package) |
...
```bash
python -c "from sama.core.Input_Data import InData; print('OK')"
```

### 3.3 Configuring Parameters Directly in Input_Data.py

Open `sama/sama/core/Input_Data.py` in your IDE. All parameters are set in the `Input_Data.__init__` method. The key sections to edit are described below.
...
## 4. Method 2 – Using the Python Package (pip install sama)

The SAMA pip package (SAMA Py Package) is available through the Python Package Index (PyPI). It provides two console commands, **`sama-config`** and **`sama-run`**, that guide you through configuration and run the optimization without editing any source files. This is the recommended approach for most users.
...
#### 4.3.1 `sama-run` command options

| Option | Description | Example |
|--------|-------------|---------|
| `-c` / `--config` | Path to config YAML (default: `sama_config_COMPLETE.yaml` in cwd) | `sama-run -c my_project.yaml` |
| `-a` / `--algorithm` | Override the algorithm from the config file | `sama-run -a pso` |
| `--output` | Override the output directory from the config | `sama-run --output results/run1` |
| `--dry-run` | Validate the config file without running optimization | `sama-run --dry-run --verbose` |
| `--no-gui` | Disable matplotlib GUI (use Agg backend for servers) | `sama-run --no-gui` |
| `--verbose` | Show full parameter log and config summary | `sama-run --verbose` |
...
After `pip install sama`, SAMA can also be used directly in Python scripts without the command-line interface:
...
# Load a YAML config produced by sama-config
...
|-----------|--------|-------|
| Particle Swarm Optimization | `from sama.optimizers.swarm import Swarm` | `Swarm()` |
| Advanced Differential Evolution | `from sama.optimizers.AdvancedDifferentialEvolution import AdvancedDifferentialEvolution` | `AdvancedDifferentialEvolution()` |
| Artificial Bee Colony | `from sama.optimizers.ArtificialBeeColony import ImprovedArtificialBeeColony` | `ImprovedArtificialBeeColony()` |
| Grey Wolf Optimizer | `from sama.optimizers.GreyWolfOptimizer import GreyWolfOptimizer` | `GreyWolfOptimizer()` |
...
The SAMA Python package is published on PyPI under the distribution name **`SAMAbyRenXera`**. This is the name you use with pip to install the package. However, once installed, the package is imported in Python code using the name **`sama`** (all lowercase). These are two different names for the same package:

**Install with pip** (use the PyPI distribution name):
```bash
pip install SAMAbyRenXera
```
...
When searching for the package on the PyPI website, look for `SAMAbyRenXera`. When writing Python scripts that use SAMA, always write `import sama`.

### 4.6 Using Your Own Custom Data Files

SAMA ships with bundled default data files for London, Ontario (weather, electrical load, and heat pump data). When you run `sama-config` and provide your own file paths, the wizard automatically copies those files into the local `sama/content/` folder in your working directory. SAMA reads from this local folder instead of the bundled defaults.

You can also copy your custom files directly into the `sama/content/` folder yourself, and SAMA will use them automatically when you run `sama-run`. The `sama/content/` folder is created in your working directory the first time you run `sama-config` or `sama-run`.
...
If a file is not present in the local `sama/content/` folder, SAMA automatically falls back to the bundled default. You only need to provide files you want to customize.
...
5. Open a terminal in your project folder and run `sama-config`. The wizard will ask for paths to your data files and copy them into `sama/content/` automatically.
...
7. Run `sama-run` to start the optimization. Results are saved in `sama_outputs/` inside your project folder.

### 4.8 Understanding the Working Directory

SAMA creates all its folders (`sama/content/`, `sama_inputs/`, `sama_outputs/`) relative to the folder where you run the `sama-config` and `sama-run` commands. This folder is called the **working directory**. Always run both commands from the same project folder so that the configuration file, custom data files, and results all stay together in one place.
...
Whether you use the wizard (`sama-config`) or edit it manually, the configuration file is a standard YAML text file. You can open it in any text editor. Below is a complete reference of all keys.
...
The following data files are bundled with the SAMA package in the `sama/content/` directory and can be accessed via `get_content_path()`:
...
|-----------------|-------|-----|
| `ModuleNotFoundError: No module named 'sama'` | SAMA not installed in current Python environment | Run `pip install sama` (or `pip install -e .` from project root) |
| `command not found: sama-config` | `pip install` did not add scripts to PATH | On Windows: restart terminal. On Linux/Mac: add `~/.local/bin` to PATH |
...
| `_tkinter.TclError: no display name` | matplotlib trying to open a GUI window on a headless server | Use `sama-run --no-gui`, or set environment variable `MPLBACKEND=Agg` |
| `FileNotFoundError: Eload.csv not found` | Package data not installed correctly | Run `pip install --force-reinstall sama` |
...
| `sama-config` freezes or no text appears | Terminal does not support questionary interactive prompts | Try a different terminal: `cmd.exe` or PowerShell on Windows; bash or zsh on Linux/macOS |
...
**Step 1.** Install SAMA (the SAMA Py Package):
...
| **`sama/cli/`** | |
| `wizard.py` | Entry point for `sama-config` command: launches `sama_Wizard.py` |
| `sama_Wizard.py` | Interactive 18-section configuration wizard using questionary |
| `runner.py` | Entry point for `sama-run` command: loads YAML, applies config, runs selected algorithm |
...
$$V_i^{t+1} = W \cdot V_i^t + c_1 U_1^t \left(P_{b_1} - P_i^t\right) + c_2 U_2^t \left(g_b^t - P_i^t\right) \tag{1}$$
...
- **Manual entry:** In `sama-config`, set `Tload_type` to 2 or higher and enter monthly average loads.
...
*SAMA is developed and maintained by the Free Appropriate Sustainability Technology (FAST) Research Group at Western University, London, Ontario, Canada.*

*Repository: https://github.com/Sas1997/SAMA | License: GPL-3.0*
