...
Tax=Tax_rate/100;
%% Constranits 
LPSP_max=0; % Maximum loss of power supply probability
RE_min=0.75;    % minimum Renewable Energy Capacity
%% Technical data
%% rated power
...
ef_bat=0.8;             % storage DC-DC efficiency (round trip efficiency)
...
Engineering_Costs=(Sales_tax+Profit_costs+Installation_cost+Overhead+Sales_and_marketing+Permiting_and_Inspection+Electrical_BoS+Structrual_BoS+Supply_Chain_costs)*(1+Tax);
%% PV
C_PV = Total_PV_price*r_PV;     % Capital cost ($) per KW
R_PV = Total_PV_price*r_PV;        % Replacement Cost of PV modules Per KW
MO_PV = 29.49*(1+Tax) ;      % PV O&M  cost ($/year/kw)
%% Inverter
C_I = Total_PV_price*r_inverter;         % Capital cost ($/kW)
R_I = Total_PV_price*r_inverter;         % Replacement cost ($/kW)
MO_I =0*(1+Tax);         %Inverter O&M cost ($/kw.year)
%% WT
C_WT = 1200*(1+Tax);      % Capital cost ($) per KW
R_WT = 1200*(1+Tax);      % Replacement Cost of WT Per KW
MO_WT = 40*(1+Tax) ;      % O&M  cost ($/year/kw)
%% Diesel generator
C_DG = 240.45*(1+Tax);       % Capital cost ($/KW)
R_DG = 240.45*(1+Tax);       % Replacement Cost ($/kW)
MO_DG = 0.064*(1+Tax);    % O&M+ running cost ($/op.h)
C_fuel=1.39*(1+Tax);  % Fuel Cost ($/L)
%% Battery
C_B = 460*(1+Tax);              % Capital cost ($/KWh)
R_B = 460*(1+Tax);              % Repalacement Cost ($/kW)
MO_B=0*(1+Tax);                % Maintenance cost ($/kw.year)
%% Charger
if Bat==1
C_CH = 149.99*(1+Tax);  % Capital Cost ($)
R_CH = 149.99*(1+Tax);  % Replacement Cost ($)
...
Engineering_Costs=(Sales_tax+Profit_costs+Installation_cost+Overhead+Sales_and_marketing+Permiting_and_Inspection+Electrical_BoS+Structrual_BoS+Supply_Chain_costs)*(1+Tax);    
%% PV
C_PV = 540*(1+Tax) ;      % Capital cost ($) per KW
R_PV = 540*(1+Tax);       % Replacement Cost of PV modules Per KW
MO_PV = 29.49*(1+Tax) ;      % O&M  cost ($/year/kw)
%% Inverter
C_I = 440*(1+Tax);        % Capital cost ($/kW)
R_I = 440*(1+Tax);        % Replacement cost ($/kW)
