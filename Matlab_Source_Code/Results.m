...
Grid_Cost= ((Annual_expenses+sum(Base_charge)+sum(Pbuy.*Cbuy)-sum(Psell.*Csell)).*1./(1+ir).^(1:n))*(Grid>0);
...
disp(['Operation Cost  = ' num2str(round(Operating_Cost,2)) ' $ '])
disp(['Initial Cost  = ' num2str(round(I_Cost,2)) ' $ '])
disp(['RE  = ' num2str(100*RE) ' % '])
disp(['Total operation and maintainance cost  = ' num2str(sum(round(MO_Cost,2))) ' $ '])
%%
