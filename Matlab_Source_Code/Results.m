...
I_Cost=C_PV*Pn_PV + C_WT*Pn_WT+ C_DG*Pn_DG+C_B*Cn_B+C_I*Cn_I +C_CH+Engineering_Costs*Pn_PV;
...
Grid_Cost= (Annual_expenses+sum(Base_charge)+sum(Pbuy.*Cbuy)-sum(Psell.*Csell) ).* 1./(1+ir).^(1:n);
...
%%%%%%%%%%%%%%%%%New edits
disp(['Total power bought from Grid= ' num2str(sum(Pbuy)) ' kWh '])
disp(['Total Money paid to the Grid= ' num2str(sum(Grid_Cost)) ' $ '])
disp(['Total Money paid by the user= ' num2str(sum(NPC)) ' $ '])
%%%%%%%%%%%%%%%%%
