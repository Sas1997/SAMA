...
import matplotlib
import os
# Agg only on headless Linux (servers, CI, Docker).
# Never force Agg on Windows or macOS — both have native GUI displays.
_is_headless = (sys.platform == 'linux' and not os.environ.get('DISPLAY', ''))
if _is_headless:
    matplotlib.use('Agg')
...
nPop = InData.nPop
MaxIt = InData.MaxIt
maxTrials = InData.maxTrials  # Increased for better exploration
modification_rate = InData.modification_rate  # Probability of modifying multiple dimensions
initial_search_radius = InData.initial_search_radius
final_search_radius = InData.final_search_radius
VarMin = InData.VarMin
VarMax = InData.VarMax

# Problem Definition
from samapy.core.Fitness import fitness as cost_function
...
class ImprovedArtificialBeeColony:
    def __init__(self, **kwargs):
        ...
        self.VarMin = VarMin * [PV, WT, Bat, DG, 1]  # Lower bound of variables
        self.VarMax = VarMax * [PV, WT, Bat, DG, 1]  # Upper bound of variables
    ...
    def optimize(self):
        ...
        plt.tight_layout()
        import os
        os.makedirs('samapy_outputs', exist_ok=True)
        plt.savefig('samapy_outputs/Optimization.png', dpi=300)
        print("✓ Saved: samapy_outputs/Optimization.png")
