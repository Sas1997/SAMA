...
==============================
Entry point for the `samapy-run` console script.

Usage
-----
    samapy-run                          # auto-finds samapy_config_COMPLETE.yaml in cwd,
                                      # uses algorithm stored in the config
    samapy-run -c my_config.yaml        # explicit config path
    samapy-run -c config.yaml -a pso    # override algorithm
    samapy-run --verbose                # show full load/patch log + config summary
    samapy-run --dry-run --verbose      # validate config without running

OUTPUT DIRECTORY ROUTING
--------------------------
Results.py exposes:  Gen_Results(X, output_dir='samapy_outputs')
The optimizers call Gen_Results() without passing output_dir, so it always
defaults to 'samapy_outputs' relative to cwd.

We redirect this WITHOUT touching Results.py or the optimizers by
monkey-patching samapy.results.Results.Gen_Results with a wrapper that
...
def _redirect_gen_results(output_dir: str):
    ...
    saved via plt.savefig with a hardcoded 'samapy_outputs/...' path.
    """
    import samapy.results.Results as _R
    ...
    # Redirect optimizer convergence plot: plt.savefig('samapy_outputs/Optimization.png')
    ...
    def _patched(fname, *args, **kwargs):
        if isinstance(fname, str):
            n = fname.replace('\\', '/')
            if n.startswith('samapy_outputs/') or n.startswith('output/figs/'):
...
def _patch_fitness(indata, verbose=False):
    import samapy.core.Fitness as _F
    d = indata
...
def _patch_results(indata, verbose=False):
    """
    Patch all module-level InData captures in samapy.results.Results.
...
def _patch_optimizer(algo, indata, verbose=False):
    ...
    try:
        if algo == 'ade':
            import samapy.optimizers.AdvancedDifferentialEvolution as _mod
            ...
            import samapy.optimizers.swarm as _mod
            _mod.PV = d.PV; _mod.WT = d.WT; _mod.Bat = d.Bat; _mod.DG = d.DG
            _mod.Run_Time = d.Run_Time; _mod.nPop = d.nPop; _mod.MaxIt = d.MaxIt
            _mod.w = d.w; _mod.wdamp = d.wdamp; _mod.c1 = d.c1; _mod.c2 = d.c2
            _mod.VarMin = d.VarMin; _mod.VarMax = d.VarMax
        elif algo == 'gwo':
            import samapy.optimizers.GreyWolfOptimizer as _mod
            _mod.PV = d.PV; _mod.WT = d.WT; _mod.Bat = d.Bat; _mod.DG = d.DG
            _mod.Run_Time = d.Run_Time; _mod.nPop = d.nPop; _mod.MaxIt = d.MaxIt
            _mod.VarMin = d.VarMin; _mod.VarMax = d.VarMax
        elif algo == 'abc':
            import samapy.optimizers.ArtificialBeeColony as _mod
            _mod.PV = d.PV; _mod.WT = d.WT; _mod.Bat = d.Bat; _mod.DG = d.DG
            _mod.Run_Time = d.Run_Time; _mod.nPop = d.nPop; _mod.MaxIt = d.MaxIt
...
def main():
    parser = argparse.ArgumentParser(
        prog='samapy-run',
        description='Run SAMAPy optimization from a wizard-generated YAML config')
    parser.add_argument('-c', '--config', default=None,
                        help='Path to YAML config (default: samapy_config_COMPLETE.yaml in cwd)')
    ...
    if (args.no_gui
            ...
            or os.environ.get('SAMAPy_HEADLESS', '0') == '1'):
        import matplotlib
        matplotlib.use('Agg')

    # ── Resolve config path ────────────────────────────────────────────────────
    config_path = Path(args.config) if args.config else Path.cwd() / 'samapy_config_COMPLETE.yaml'
    if not config_path.exists():
        print(f"❌ Config not found: {config_path}")
        print("Run  samapy-config  first to generate a configuration file.")
        sys.exit(1)
    ...
    pd.DataFrame(data).to_csv('samapy_inputs/Inputs.csv', index=False)
    ...
    # ── Output directory ───────────────────────────────────────────────────────
    # Priority: --output flag  >  config output_directory  >  'samapy_outputs'
    out_dir = args.output or config.get('output_directory', 'samapy_outputs')
