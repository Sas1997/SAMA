...
def Gen_Results(X):
    ...
    if Nbat > 0:
        fig, ax = plt.subplots(figsize=(20, 10), dpi=300)  # Increased figure size and resolution
    ...
    fig, axs = plt.subplots(n_rows, n_cols, figsize=(16, n_rows * 4), dpi=300)  # Adjusting figure size based on number of rows
    ...
    if np.sum(Psell) > 0.1:
        ...
        ax2.tick_params(labelsize=22)
        # Place the title below the color bar
        ax2.set_title('Daily average Sell to the Grid [$]', fontsize=32, y=-1.35)
        # Add yearly sum as a new cell at the bottom right of the figure
        pos = ax2.get_position()
        pos = [pos.x1 + 0.035, pos.y0 - 0.035, 0.08, pos.height]
        ax3 = fig.add_axes(pos)
        ...
        cbar_total = plt.colorbar(ax1.collections[0], cax=cax, orientation='vertical')
        cbar_total.ax.tick_params(labelsize=22)
        cbar_total.ax.set_title('Monthly average Sell to the Grid [$]', fontsize=32, rotation=270, x=3.5, y=0.225)
        fig.subplots_adjust(left=0.075, top=0.98, bottom=0.075)
