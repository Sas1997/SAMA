...
from samapy.ems.EMS_EV import EMS_EV
from samapy.utilities.EV_travel import compute_ev_travel_energy
from samapy.utilities.EV_travel import calculate_energy_consumption
...
def Gen_Results(X, output_dir=None):
    ...
    output_dir : str
        Directory where outputs will be saved (default: 'samapy_outputs')
    """

    if output_dir is None:
        output_dir = OUTPUT_DIR

    # Create simple output directory structure (no timestamps)
    ...
    if EV > 0:
        Pdg, Ens, Pbuy, Psell, Edump, Pch, Pdch, Eb, Pdch_max, Pch_max, Pinv, Pev_ch, Pev_dch, Eev, Ech_req, EV_dem, price_dg, Cbw, Cevw = EMS_EV(
        ...
        EV_dem = 0
        Cevw = np.inf
        Pdg, Ens, Pbuy, Psell, Edump, Pch, Pdch, Eb, Pdch_max, Pch_max, price_dg, Cbw = EMS(Lead_acid, Li_ion, Ich_max_Li_ion,
    ...
    from samapy.ems.EMS_HP import calculate_hp_grid_allocation, calculate_hp_energy_sources_economic
    # Pbuy_eH, Pbuy_C, Pbuy_o, balance = calculate_hp_grid_allocation(
    ...
    if HP == 0:
        ...
        Pbuy_eH = np.zeros(NT)
        Pbuy_C = np.zeros(NT)
        cost_breakdown = {'RE': 0, 'DG': 0, 'Battery': 0, 'EV': 0, 'Grid': 0, 'Total': 0}
        balance_method2 = np.zeros(NT)
        HP_curtailed = np.zeros(NT)
    else:
        (HP_RE, HP_DG, HP_Batt, HP_EV, HP_Grid, Pbuy_eH, Pbuy_C,
    ...
    NG_Grid_Cost_onlyG = (((Annual_expenses_NG + np.sum(Service_charge_NG) + np.sum(Hload * Cbuy_NG) + Grid_Tax_amount_NG * np.sum(Hload)) * (cumulative_escalation_NG / ((1 + ir) ** np.arange(1, n + 1)))) * (1 + Grid_Tax_NG) - ((Grid_credit_NG) * (cumulative_escalation_NG / ((1 + ir) ** np.arange(1, n + 1))))) * (HP > 0)
    ...
    avoided_costs_ng = (((np.sum(Hload * Cbuy_NG) + Annual_expenses_NG + np.sum(Service_charge_NG) + Grid_Tax_amount_NG * np.sum(Hload)) * (cumulative_escalation_NG / ((1 + ir) ** np.arange(1, n + 1))) * (1 + Grid_Tax_NG)) - (Grid_credit_NG * (cumulative_escalation_NG / ((1 + ir) ** np.arange(1, n + 1))))) * (HP > 0)
    ...
    (HP == 1) and print("Selected heat pump based on the maximum thermal load: Heat Pump Model ", HP_model, " with the capacity of ", Pn_hp, " BTU/hr")
    ...
    # HP new parameters
    print('Annual Heating Load =', np.sum(Hload), 'kWh')
    print('Annual Cooling Load =', np.sum(Cload), 'kWh')
    print('Annual Thermal Load =', np.sum(Hload + Cload), 'kWh')
    print('HP Annual Heating Required Power =', np.sum(power_hp_heating), 'kWh')
    print('HP Annual Cooling Required Power =', np.sum(power_hp_cooling), 'kWh')
    print('HP Average heating COP =', np.nanmean(COP_hp_heating), 'kWh')
    print('HP Minimum heating COP =', np.nanmin(COP_hp_heating), 'kWh')
    print('HP Maximum heating COP =', np.nanmax(COP_hp_heating), 'kWh')
    print('HP Average cooling COP =', np.nanmean(COP_hp_cooling), 'kWh')
    print('HP Minimum cooling COP =', np.nanmin(COP_hp_cooling), 'kWh')
    print('HP Maximum cooling COP =', np.nanmax(COP_hp_cooling), 'kWh')
    print('HP Annual Power bought from Grid (for heating) =', np.sum(Pbuy_eH), 'kWh')
    print('HP Annual Power bought from Grid (for cooling) =', np.sum(Pbuy_C), 'kWh')
    print('HP Annual Power bought from Grid =', np.sum(Pbuy_HP), 'kWh')
    print('HP Annual Power supplied by Other Sources (for heating) =', np.sum(power_hp_heating) - np.sum(Pbuy_eH), 'kWh')
    print('HP Annual Power supplied by Other Sources (for cooling) =', np.sum(power_hp_cooling) - np.sum(Pbuy_C),'kWh')
    print('HP Annual Power supplied by Other Sources =', np.sum(power_hp_heating) + np.sum(power_hp_cooling) - np.sum(Pbuy_HP), 'kWh')
    ...
    plt.tight_layout()
    fig_path = os.path.join(figs_dir, 'Cash_Flow.svg')
    ...
    plt.ylim(y_min - y_margin, y_max + y_margin)
    plt.tight_layout()
    plt.savefig(os.path.join(figs_dir, 'Cash Flow_ADV.png'), dpi=300)
    plt.close()
    ...
    if Cash_Flow_adv == 1:
        ...
        plt.ylim(y_min - y_margin, y_max + y_margin)
        plt.tight_layout()
        plt.savefig(os.path.join(figs_dir, 'Multiple_Cash_Flow_ADV.png'), dpi=300)
        plt.close()

    from samapy.pricing.Electricity_Bill_Calculator import compare_scenarios, print_comparison

    Pbuy_no_sell = Eload + EV_dem  # No generation, buy everything
    Pbuy_with_sell = Pbuy  # With generation, buy only net demand

    # Run comparison
    comparison = compare_scenarios(daysInMonth, Pbuy_no_sell, Pbuy_with_sell, Cbuy, Service_charge,
                                   Grid_Tax, Grid_Tax_amount, Annual_expenses,
                                   Grid_escalation, Grid_credit, NEM_fee,
                                   Grid, n, ir, Psell, Csell,
                                   HP, Hload, Cbuy_NG, Service_charge_NG,
                                   Grid_Tax_NG, Grid_Tax_amount_NG, Annual_expenses_NG,
                                   Grid_escalation_NG, Grid_credit_NG)
    # Print comparison
    print_comparison(comparison, show_plot=True, save_path=os.path.join(figs_dir, 'electricity_comparison.png'))
    ...
    if Grid == 0:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(figs_dir, 'Grid Interconnection.png'), dpi=300)
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(figs_dir, 'Energy Distribution.png'), dpi=300)
    plt.close()
    ...
    if Nbat > 0:
        ...
        ax.plot(Eb / Cn_B if Cn_B > 0 else np.zeros_like(Eb), marker='.', linestyle='-', linewidth=0.5, color='blue')
        ...
        # Set x-axis and y-axis to start from zero
        ax.set_xlim([0, max(hours_per_month) + 744])
        ax.set_ylim([0, max(Eb / Cn_B if Cn_B > 0 else np.zeros_like(Eb))])
        # Adjust the margins and space between subplots
        plt.subplots_adjust(left=0.08, right=0.95, top=0.95, bottom=0.08)
        plt.tight_layout()
        plt.savefig(os.path.join(figs_dir, 'Battery State of Charge.png'), dpi=300)
        plt.close()
    ...
    plt.savefig(os.path.join(figs_dir, 'Specific day results.png'), dpi=300)
    plt.close()
    ...
    plt.savefig(os.path.join(figs_dir, 'Daily-Monthly-Yearly average cost of energy system.png'), dpi=300)
    plt.close()
    ...
    plt.savefig(os.path.join(figs_dir, 'Daily-Monthly-Yearly average hourly cost of connecting to the grid.png'), dpi=300)
    plt.close()
    ...
    fig.subplots_adjust(left=0.075, top=0.98, bottom=0.075)
    plt.savefig(os.path.join(figs_dir, 'Daily-Monthly-Yearly average cost of only grid-connected system.png'), dpi=300)
    plt.close()
    ...
    ax.set_xticks(hours_per_month)
    ax.set_xticklabels(month_labels)
    plt.savefig(os.path.join(figs_dir, 'Grid Hourly Cost.png'), dpi=300)
    plt.close()
    ...
    if EV > 0 :
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(figs_dir, 'Energy Balance.png'), dpi=300)
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(figs_dir, 'EV Energy.png'), dpi=300)
        plt.close()
        ...
        # ─────── Save the figure ───────
        plt.tight_layout()
        plt.savefig(os.path.join(figs_dir, 'EV Sp Results 1.png'), dpi=300)
        plt.close()
        ...
        # ─────── Save the figure ───────
        plt.tight_layout()
        plt.savefig(os.path.join(figs_dir, 'EV Sp Results 2.png'), dpi=300)
        plt.close()

    if HP > 0:
        # Set style with Times New Roman and larger fonts
        ...
        def plot_ambient_conditions(T, P, save_path=None):
            """
            Plot ambient temperature and pressure profiles
            """
            # Create time index
            start_date = datetime(2024, 1, 1)
            time_index = [start_date + timedelta(hours=i) for i in range(len(T))]

            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 11))

            colors = {'temp': '#ff6b6b', 'pressure': '#4ecdc4'}

            # Temperature Profile
            ax1.set_axisbelow(True)
            ax1.plot(time_index, T, color=colors['temp'], linewidth=1.5, alpha=0.8)
            ax1.fill_between(time_index, T, alpha=0.3, color=colors['temp'])
            ax1.set_ylabel('Temperature [°C]', fontweight='bold', fontsize=16)
            ax1.set_title('a) Ambient Temperature Profile', fontweight='bold', fontsize=18, loc='left')
            ax1.grid(True, alpha=0.3)
            ax1.xaxis.set_major_formatter(mdates.DateFormatter('%b'))
            ax1.xaxis.set_major_locator(mdates.MonthLocator())
            ax1.tick_params(axis='both', which='major', labelsize=14)
            ax1.text(0.02, 0.98, f'Max: {np.max(T):.1f}°C\nMin: {np.min(T):.1f}°C\nAvg: {np.mean(T):.1f}°C',
                     transform=ax1.transAxes, verticalalignment='top',
                     bbox=dict(boxstyle='round', facecolor='white', alpha=0.9), fontsize=13)

            # Pressure Profile
            ax2.set_axisbelow(True)
            ax2.plot(time_index, P, color=colors['pressure'], linewidth=1.5, alpha=0.8)
            ax2.fill_between(time_index, P, alpha=0.3, color=colors['pressure'])
            ax2.set_ylabel('Pressure [mbar]', fontweight='bold', fontsize=16)
            ax2.set_xlabel('Month', fontweight='bold', fontsize=16)
            ax2.set_title('b) Ambient Pressure Profile', fontweight='bold', fontsize=18, loc='left')
            ax2.grid(True, alpha=0.3)
            ax2.xaxis.set_major_formatter(mdates.DateFormatter('%b'))
            ax2.xaxis.set_major_locator(mdates.MonthLocator())
            ax2.tick_params(axis='both', which='major', labelsize=14)
            ax2.text(0.02, 0.98, f'Max: {np.max(P):.0f} mbar\nMin: {np.min(P):.0f} mbar\nAvg: {np.mean(P):.0f} mbar',
        ...
        def plot_heating_performance(T, Hload, power_hp_heating, COP_hp_heating, save_path=None):
            """
            Plot heating load vs HP power and heating COP
            """
            start_date = datetime(2024, 1, 1)
            time_index = [start_date + timedelta(hours=i) for i in range(len(T))]
            ...
            colors = {'heating': '#ff8c42', 'power': '#9b59b6', 'cop_h': '#f39c12'}

            # Heating Load vs HP Power
            ax1.set_axisbelow(True)
            ax1.plot(time_index, Hload, color=colors['heating'], linewidth=1.5, alpha=0.7, label='Heating Load')
            ax1.plot(time_index, power_hp_heating, color=colors['power'], linewidth=1.5, alpha=0.7, label='HP Power')
            ax1.fill_between(time_index, Hload, alpha=0.2, color=colors['heating'])
            ax1.fill_between(time_index, power_hp_heating, alpha=0.2, color=colors['power'])
            ax1.set_ylabel('Power [kW]', fontweight='bold', fontsize=16)
            ax1.set_title('a) Heating Load vs HP Power', fontweight='bold', fontsize=18, loc='left')
            ...
            total_heat = np.sum(Hload)
            total_power_h = np.sum(power_hp_heating)
            ax1.text(0.02, 0.98, f'Total Heat: {total_heat:,.0f} kWh\nTotal Power: {total_power_h:,.0f} kWh',
                     transform=ax1.transAxes, verticalalignment='top',
                     bbox=dict(boxstyle='round', facecolor='white', alpha=0.9), fontsize=13)

            # Heating COP
            ax2.set_axisbelow(True)
            valid_heating = Hload > 0.1
            if np.any(valid_heating):
                ax2.scatter(time_index, COP_hp_heating, c=T, cmap='coolwarm', s=15, alpha=0.5, edgecolors='none')
                ax2.axhline(y=np.nanmean(COP_hp_heating[valid_heating]), color='red', linestyle='--',
                            linewidth=2.5, label=f'Avg: {np.nanmean(COP_hp_heating[valid_heating]):.2f}')
                ax2.text(0.02, 0.98,
                         f'Max: {np.nanmax(COP_hp_heating[valid_heating]):.2f}\nMin: {np.nanmin(COP_hp_heating[valid_heating]):.2f}',
                         transform=ax2.transAxes, verticalalignment='top',
                         bbox=dict(boxstyle='round', facecolor='white', alpha=0.9), fontsize=13)
            ax2.set_ylabel('COP', fontweight='bold', fontsize=16)
            ax2.set_xlabel('Month', fontweight='bold', fontsize=16)
            ax2.set_title('b) Heating COP (colored by temperature)', fontweight='bold', fontsize=18, loc='left')
            y_max = max(6, np.percentile(COP_hp_heating[valid_heating], 99)) if np.any(valid_heating) else 6
        ...
        def plot_cooling_performance(T, Cload, power_hp_cooling, COP_hp_cooling, save_path=None):
            """
            Plot cooling load vs HP power and cooling COP
            """
            start_date = datetime(2024, 1, 1)
            time_index = [start_date + timedelta(hours=i) for i in range(len(T))]
            ...
            colors = {'cooling': '#4a90e2', 'power': '#9b59b6', 'cop_c': '#3498db'}

            # Cooling Load vs HP Power
            ax1.set_axisbelow(True)
            ax1.plot(time_index, Cload, color=colors['cooling'], linewidth=1.5, alpha=0.7, label='Cooling Load')
            ax1.plot(time_index, power_hp_cooling, color=colors['power'], linewidth=1.5, alpha=0.7, label='HP Power')
            ax1.fill_between(time_index, Cload, alpha=0.2, color=colors['cooling'])
            ax1.fill_between(time_index, power_hp_cooling, alpha=0.2, color=colors['power'])
            ax1.set_ylabel('Power [kW]', fontweight='bold', fontsize=16)
            ax1.set_title('c) Cooling Load vs HP Power', fontweight='bold', fontsize=18, loc='left')
            ax1.legend(loc='upper right', framealpha=0.9, fontsize=14)
            ax1.grid(True, alpha=0.3)
            ax1.xaxis.set_major_formatter(mdates.DateFormatter('%b'))
            ax1.xaxis.set_major_locator(mdates.MonthLocator())
            ax1.tick_params(axis='both', which='major', labelsize=14)
            ax1.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:,.1f}'))
            total_cool = np.sum(Cload)
            total_power_c = np.sum(power_hp_cooling)
            ax1.text(0.02, 0.98, f'Total Cooling: {total_cool:,.0f} kWh\nTotal Power: {total_power_c:,.0f} kWh',
                     transform=ax1.transAxes, verticalalignment='top',
                     bbox=dict(boxstyle='round', facecolor='white', alpha=0.9), fontsize=13)

            # Cooling COP
            ax2.set_axisbelow(True)
            valid_cooling = Cload > 0.1
            if np.any(valid_cooling):
                ax2.scatter(time_index, COP_hp_cooling, c=T, cmap='coolwarm', s=15, alpha=0.5, edgecolors='none')
                ax2.axhline(y=np.nanmean(COP_hp_cooling[valid_cooling]), color='blue', linestyle='--',
                            linewidth=2.5, label=f'Avg: {np.nanmean(COP_hp_cooling[valid_cooling]):.2f}')
                ax2.text(0.02, 0.98,
                         f'Max: {np.nanmax(COP_hp_cooling[valid_cooling]):.2f}\nMin: {np.nanmin(COP_hp_cooling[valid_cooling]):.2f}',
                         transform=ax2.transAxes, verticalalignment='top',
                         bbox=dict(boxstyle='round', facecolor='white', alpha=0.9), fontsize=13)
            ax2.set_ylabel('COP', fontweight='bold', fontsize=16)
            ax2.set_xlabel('Month', fontweight='bold', fontsize=16)
            ax2.set_title('d) Cooling COP (colored by temperature)', fontweight='bold', fontsize=18, loc='left')
            y_max = max(6, np.percentile(COP_hp_cooling[valid_cooling], 99)) if np.any(valid_cooling) else 6
            ax2.set_ylim([0, y_max * 1.2])  # 1.2 adds 10% padding at the top
            ax2.legend(loc='upper right', framealpha=0.9, fontsize=14)
            ax2.grid(True, alpha=0.3)
            ax2.xaxis.set_major_formatter(mdates.DateFormatter('%b'))
            ax2.xaxis.set_major_locator(mdates.MonthLocator())
        ...
        def plot_cop_vs_temperature(T, Hload, Cload, COP_hp_heating, COP_hp_cooling, save_path=None):
            """
            Plot COP vs Temperature for both heating and cooling with professional visualization
            """
            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 12))

            # Heating COP vs Temperature
            ax1.set_axisbelow(True)
            valid_h = (Hload > 0.1) & (COP_hp_heating > 0) & (COP_hp_heating < 10)
            if np.any(valid_h):
                # Convert to numpy arrays
                T_valid = np.array(T[valid_h])
                COP_valid = np.array(COP_hp_heating[valid_h])
                Hload_valid = np.array(Hload[valid_h])

                # Create scatter plot colored by heating load
                scatter = ax1.scatter(T_valid, COP_valid, c=Hload_valid, cmap='Reds',
                                      s=40, alpha=0.6, edgecolors='none')

                # Add colorbar
                cb1 = plt.colorbar(scatter, ax=ax1, pad=0.02)
                cb1.set_label('Heating Load [kW]', fontweight='bold', fontsize=14)

                # Calculate and plot binned average
                T_bins = np.linspace(T_valid.min(), T_valid.max(), 20)
                T_binned = []
                COP_binned = []

                for i in range(len(T_bins) - 1):
                    mask = (T_valid >= T_bins[i]) & (T_valid < T_bins[i + 1])
                    if np.any(mask):
                        T_binned.append((T_bins[i] + T_bins[i + 1]) / 2)
                        COP_binned.append(np.mean(COP_valid[mask]))

                # Plot trend line
                ax1.plot(T_binned, COP_binned, 'o-', color='#2c3e50', linewidth=3,
                         markersize=8, label='Average COP', zorder=10,
                         markeredgecolor='white', markeredgewidth=2)

                # Add statistics
                mean_cop = np.mean(COP_valid)
                ax1.axhline(y=mean_cop, color='#e74c3c', linestyle='--', linewidth=2,
                            alpha=0.7, label=f'Mean: {mean_cop:.2f}')

                # Statistics box - positioned to avoid data
                stats_text = (f'Mean COP: {mean_cop:.2f}  |  Std: {np.std(COP_valid):.2f}\n'
                              f'Range: {COP_valid.min():.2f} – {COP_valid.max():.2f}')
                ax1.text(0.02, 0.98, stats_text, transform=ax1.transAxes,
                         verticalalignment='top', horizontalalignment='left',
                         fontsize=14, bbox=dict(boxstyle='round,pad=0.6',
                                                facecolor='white', alpha=0.92, edgecolor='#c0392b', linewidth=1.5))

            ax1.set_xlabel('Ambient Temperature [°C]', fontweight='bold', fontsize=16)
            ax1.set_ylabel('Heating COP [-]', fontweight='bold', fontsize=16)
            ax1.set_title('a) Heating COP vs Temperature', fontsize=18, fontweight='bold', loc='left')
            ax1.legend(loc='lower right', fontsize=14, framealpha=0.92)
            ax1.grid(True, alpha=0.3, linestyle='--', linewidth=1)
            ax1.set_facecolor('#fafafa')
            ax1.tick_params(axis='both', which='major', labelsize=14)

            # Cooling COP vs Temperature
            ax2.set_axisbelow(True)
            valid_c = (Cload > 0.1) & (COP_hp_cooling > 0) & (COP_hp_cooling < 10)
            if np.any(valid_c):
                # Convert to numpy arrays
                T_valid = np.array(T[valid_c])
                COP_valid = np.array(COP_hp_cooling[valid_c])
                Cload_valid = np.array(Cload[valid_c])

                # Create scatter plot colored by cooling load
                scatter = ax2.scatter(T_valid, COP_valid, c=Cload_valid, cmap='Blues',
                                      s=40, alpha=0.6, edgecolors='none')

                # Add colorbar
                cb2 = plt.colorbar(scatter, ax=ax2, pad=0.02)
                cb2.set_label('Cooling Load [kW]', fontweight='bold', fontsize=14)
                ...
                # Plot trend line
                ax2.plot(T_binned, COP_binned, 'o-', color='#2c3e50', linewidth=3,
                         markersize=8, label='Average COP', zorder=10,
                         markeredgecolor='white', markeredgewidth=2)

                # Add statistics
                mean_cop = np.mean(COP_valid)
                ax2.axhline(y=mean_cop, color='#3498db', linestyle='--', linewidth=2,
                            alpha=0.7, label=f'Mean: {mean_cop:.2f}')
                ...
                ax2.text(0.02, 0.98, stats_text, transform=ax2.transAxes,
                         ...
                         fontsize=14, bbox=dict(boxstyle='round,pad=0.6',
                                                facecolor='white', alpha=0.92, edgecolor='#2980b9', linewidth=1.5))

            ax2.set_xlabel('Ambient Temperature [°C]', fontweight='bold', fontsize=16)
            ax2.set_ylabel('Cooling COP [-]', fontweight='bold', fontsize=16)
            ax2.set_title('b) Cooling COP vs Temperature', fontsize=18, fontweight='bold', loc='left')
            ax2.legend(loc='lower right', fontsize=14, framealpha=0.92)
        ...
        def plot_monthly_summary(T, Hload, Cload, power_hp_heating, power_hp_cooling,
                                 COP_hp_heating, COP_hp_cooling, save_path=None):
            """
            ...
            for hours in hours_per_month:
                end_idx = idx + hours
                monthly_heat_load.append(np.sum(Hload[idx:end_idx]))
                monthly_cool_load.append(np.sum(Cload[idx:end_idx]))
                monthly_heat_power.append(np.sum(power_hp_heating[idx:end_idx]))
                monthly_cool_power.append(np.sum(power_hp_cooling[idx:end_idx]))

                if np.sum(Hload[idx:end_idx]) > 0:
                    valid_h = Hload[idx:end_idx] > 0.1
                    monthly_heat_cop.append(np.nanmean(COP_hp_heating[idx:end_idx][valid_h]) if np.any(valid_h) else 0)
                else:
                    monthly_heat_cop.append(0)

                if np.sum(Cload[idx:end_idx]) > 0:
                    valid_c = Cload[idx:end_idx] > 0.1
                    monthly_cool_cop.append(np.nanmean(COP_hp_cooling[idx:end_idx][valid_c]) if np.any(valid_c) else 0)
                else:
                    monthly_cool_cop.append(0)

                monthly_temp.append(np.nanmean(T[idx:end_idx]))
                idx = end_idx
            ...
            ax1 = axes[0, 0]
            ax1.set_axisbelow(True)
            ax1.bar(x - width / 2, monthly_heat_load, width, label='Heating Load', color='#ff8c42', alpha=0.8)
            ax1.bar(x + width / 2, monthly_cool_load, width, label='Cooling Load', color='#4a90e2', alpha=0.8)
            ax1.set_ylabel('Energy [kWh]', fontweight='bold', fontsize=16)
            ax1.set_title('a) Monthly Heating and Cooling Load', fontweight='bold', fontsize=18, loc='left')
            ax1.set_xticks(x)
            ax1.set_xticklabels(months, rotation=45, fontsize=14)
            ax1.legend(fontsize=14)
            ax1.grid(True, alpha=0.3, axis='y')
            ax1.tick_params(axis='both', which='major', labelsize=14)
            ax1.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:,.0f}'))

            # Monthly HP Power Consumption
            ax2 = axes[0, 1]
            ax2.set_axisbelow(True)
            ax2.bar(x - width / 2, monthly_heat_power, width, label='Heating Power', color='#e74c3c', alpha=0.8)
            ax2.bar(x + width / 2, monthly_cool_power, width, label='Cooling Power', color='#3498db', alpha=0.8)
            ax2.set_ylabel('Energy [kWh]', fontweight='bold', fontsize=16)
            ax2.set_title('b) Monthly HP Power Consumption', fontweight='bold', fontsize=18, loc='left')
            ax2.set_xticks(x)
            ax2.set_xticklabels(months, rotation=45, fontsize=14)
            ...
            monthly_heat_cop_clean = [cop if cop > 0 else np.nan for cop in monthly_heat_cop]
            monthly_cool_cop_clean = [cop if cop > 0 else np.nan for cop in monthly_cool_cop]
            ax3.bar(x - width / 2, monthly_heat_cop_clean, width, label='Heating COP', color='#f39c12', alpha=0.8)
            ax3.bar(x + width / 2, monthly_cool_cop_clean, width, label='Cooling COP', color='#1abc9c', alpha=0.8)
            ax3.set_ylabel('COP', fontweight='bold', fontsize=16)
            ax3.set_title('c) Monthly Average COP', fontweight='bold', fontsize=18, loc='left')
            ax3.set_xticks(x)
            ax3.set_xticklabels(months, rotation=45, fontsize=14)
            ...
            ax4.bar(x, monthly_temp, color='#ff6b6b', alpha=0.8)
            ax4.set_ylabel('Temperature [°C]', fontweight='bold', fontsize=16)
            ax4.set_title('d) Monthly Average Temperature', fontweight='bold', fontsize=18, loc='left')
            ax4.set_xticks(x)
            ax4.set_xticklabels(months, rotation=45, fontsize=14)
        ...
        # Create all plots
        plot_ambient_conditions(T, P, save_path=os.path.join(figs_dir, 'hp_ambient_conditions.png'))
        plot_heating_performance(T, Hload, power_hp_heating, COP_hp_heating,
                                 save_path=os.path.join(figs_dir, 'hp_heating_performance.png'))
        plot_cooling_performance(T, Cload, power_hp_cooling, COP_hp_cooling,
                                 save_path=os.path.join(figs_dir, 'hp_cooling_performance.png'))
        plot_cop_vs_temperature(T, Hload, Cload, COP_hp_heating, COP_hp_cooling,
                                save_path=os.path.join(figs_dir, 'hp_cop_vs_temp.png'))
        plot_monthly_summary(T, Hload, Cload, power_hp_heating, power_hp_cooling,
                             COP_hp_heating, COP_hp_cooling, save_path=os.path.join(figs_dir, 'hp_monthly_summary.png'))

    plt.close('all')

    # Copy results to default samapy_outputs folder as well
    import shutil
    _default = 'samapy_outputs'
    if os.path.abspath(output_dir) != os.path.abspath(_default):
        if os.path.exists(_default):
            shutil.rmtree(_default)
        shutil.copytree(output_dir, _default)
        print(f'\U0001f4c2 Also copied to: {os.path.abspath(_default)}')
