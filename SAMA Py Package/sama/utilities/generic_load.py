...
def generic_load(load_type, load_previous_year_type, peakmonth, daysInMonth, user_defined_load):
    if load_type == 8 or load_previous_year_type == 9:
        if peakmonth == 'July':
            path_generic_load = get_content_path('Generic_load_JulyP.csv')
        else:
            path_generic_load = get_content_path('Generic_load_JanuaryP.csv')
    ...
    elif load_type == 10 or load_previous_year_type == 11:
        # New load type: daily scaling (user_defined_load is 365 daily totals)
        if peakmonth == 'July':
            path_generic_load = get_content_path('Generic_load_JulyP.csv')
        else:
            path_generic_load = get_content_path('Generic_load_JanuaryP.csv')
        ...
        scaled_data = pd.Series(dtype=float)
        for day in range(np.sum(daysInMonth)):
            start = day * 24
            end = start + 24
            daily_load = Eload[start:end]
            scaling_factor = user_defined_load[day] / np.sum(daily_load)
            scaled_day = daily_load * scaling_factor
            scaled_data = pd.concat([scaled_data, pd.Series(scaled_day)], axis=0)
