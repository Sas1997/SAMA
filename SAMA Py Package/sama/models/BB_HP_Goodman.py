...
def Heat_Pump_Model(T, P, Hload, Cload):
    ...
    model_capacity = np.genfromtxt(get_content_path('HP_Goodman/models.csv'), delimiter=',', skip_header=1, dtype=str)
    heating_COP = np.genfromtxt(get_content_path('HP_Goodman/heating_COP.csv'), delimiter=',', skip_header=1)
    cooling_COP = np.genfromtxt(get_content_path('HP_Goodman/cooling_COP.csv'), delimiter=',', skip_header=1)
