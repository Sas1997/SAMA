...
"""
SAMA MASTER CONFIGURATION WIZARD
...
def copy_to_content(source_file, target_filename):
    """Copy user file to sama/sama/content/ folder"""
    content_dir = Path.cwd() / 'sama' / 'content'
...
def setup_directories():
    ...
    dirs = {'output_dir': questionary.text("Where should results be saved?", default=str(Path.cwd() / 'sama_outputs')).ask()}
...
""")
    ...
    output_path = input_dir / 'sama_config_COMPLETE.yaml'

    with open(output_path, 'w') as f:
        yaml.dump(config, f, default_flow_style=False, sort_keys=False)

    print(f"\n✅ Configuration saved → {output_path}")
    print(f"\n   Run optimization:  sama-run\n")
