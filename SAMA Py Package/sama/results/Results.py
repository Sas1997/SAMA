...
from sama.ems.EMS_EV import EMS_EV
from sama.utilities.EV_travel import compute_ev_travel_energy
from sama.utilities.EV_travel import calculate_energy_consumption
...
def Gen_Results(X, output_dir=None):
    ...
    output_dir : str
        Directory where outputs will be saved (default: 'sama_outputs')
    """
    ...
    from sama.ems.EMS_HP import calculate_hp_grid_allocation, calculate_hp_energy_sources_economic
    ...
    print('HP Average heating COP =', np.mean(COP_hp_heating), 'kWh')
    print('HP Minimum heating COP =', np.min(COP_hp_heating), 'kWh')
    print('HP Maximum heating COP =', np.max(COP_hp_heating), 'kWh')
    print('HP Average cooling COP =', np.mean(COP_hp_cooling), 'kWh')
    print('HP Minimum cooling COP =', np.min(COP_hp_cooling), 'kWh')
    print('HP Maximum cooling COP =', np.max(COP_hp_cooling), 'kWh')
    ...
    from sama.pricing.Electricity_Bill_Calculator import compare_scenarios, print_comparison
    ...
    if HP > 0:
        ...
        def plot_ambient_conditions(T, P, save_path=None):
            ...
            ax1.set_title('a)', fontweight='bold', fontsize=18, loc='left')
            ...
            ax2.set_title('b)', fontweight='bold', fontsize=18, loc='left')
        ...
        def plot_heating_performance(T, Hload, power_hp_heating, COP_hp_heating, save_path=None):
            ...
            if np.any(valid_heating):
                ...
                ax2.axhline(y=np.mean(COP_hp_heating[valid_heating]), color='red', linestyle='--',
                            linewidth=2.5, label=f'Avg: {np.mean(COP_hp_heating[valid_heating]):.2f}')
                ax2.text(0.02, 0.98,
                         f'Max: {np.max(COP_hp_heating[valid_heating]):.2f}\nMin: {np.min(COP_hp_heating[valid_heating]):.2f}',
                         transform=ax2.transAxes, verticalalignment='top',
                         bbox=dict(boxstyle='round', facecolor='white', alpha=0.9), fontsize=13)
            ax2.set_ylabel('COP', fontweight='bold', fontsize=16)
            ax2.set_xlabel('Month', fontweight='bold', fontsize=16)
            ax2.set_title('b)', fontweight='bold', fontsize=18, loc='left')
            ax2.set_ylim([0, max(6, np.percentile(COP_hp_heating[valid_heating], 99)) if np.any(valid_heating) else 6])
        ...
        def plot_cooling_performance(T, Cload, power_hp_cooling, COP_hp_cooling, save_path=None):
            ...
            if np.any(valid_cooling):
                ...
                ax2.axhline(y=np.mean(COP_hp_cooling[valid_cooling]), color='blue', linestyle='--',
                            linewidth=2.5, label=f'Avg: {np.mean(COP_hp_cooling[valid_cooling]):.2f}')
                ax2.text(0.02, 0.98,
                         f'Max: {np.max(COP_hp_cooling[valid_cooling]):.2f}\nMin: {np.min(COP_hp_cooling[valid_cooling]):.2f}',
                         transform=ax2.transAxes, verticalalignment='top',
                         bbox=dict(boxstyle='round', facecolor='white', alpha=0.9), fontsize=13)
            ax2.set_ylabel('COP', fontweight='bold', fontsize=16)
            ax2.set_xlabel('Month', fontweight='bold', fontsize=16)
            ax2.set_title('b)', fontweight='bold', fontsize=18, loc='left')
            ax2.set_ylim([0, max(6, np.percentile(COP_hp_cooling[valid_cooling], 99)) if np.any(valid_cooling) else 6])
            ax2.legend(loc='upper right', framealpha=0.9, fontsize=14)
            ax2.grid(True, alpha=0.3)
            ax2.xaxis.set_major_formatter(mdates.DateFormatter('%b'))
            ax2.xaxis.set_major_locator(mdates.MonthLocator())
        ...
        def plot_monthly_summary(T, Hload, Cload, power_hp_heating, power_hp_cooling,
            ...
            for hours in hours_per_month:
                ...
                if np.sum(Hload[idx:end_idx]) > 0:
                    ...
                    monthly_heat_cop.append(np.mean(COP_hp_heating[idx:end_idx][valid_h]) if np.any(valid_h) else 0)
                else:
                    monthly_heat_cop.append(0)

                if np.sum(Cload[idx:end_idx]) > 0:
                    valid_c = Cload[idx:end_idx] > 0.1
                    monthly_cool_cop.append(np.mean(COP_hp_cooling[idx:end_idx][valid_c]) if np.any(valid_c) else 0)
                else:
                    monthly_cool_cop.append(0)

                monthly_temp.append(np.mean(T[idx:end_idx]))
                idx = end_idx
            ...
            ax3.set_title('c)', fontweight='bold', fontsize=18, loc='left')
            ax3.set_xticks(x)
            ...
            ax4.set_title('d)', fontweight='bold', fontsize=18, loc='left')
            ax4.set_xticks(x)
    ...
    # Copy results to default sama_outputs folder as well
    import shutil
    _default = 'sama_outputs'
