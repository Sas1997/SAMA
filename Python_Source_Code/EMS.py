...
def energy_management(
    ...
    for t in range(NT):
        Pdch_max, Pch_max = battery_model(Nbat, Eb[t], inputs.alfa_battery, inputs.c, inputs.k, inputs.Imax, inputs.Vnom, inputs.ef_bat) # kW
        ...
        # if load greater than PV+Pwt 
            ...
            if (inputs.Cbuy[t] <= price_dg) and (price_dg <= Cbw): # Grid, DG , Bat : 1
                ...
                Eb_e=(Eb[t]-Ebmin)*inputs.ef_bat
                Pdch[t]=min(Eb_e,Edef_DC)*int(Edef_DC>0)
                Pdch[t]=min(Pdch[t],Pdch_max) 
