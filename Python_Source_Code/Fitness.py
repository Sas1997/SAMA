...
def CostFunctionMatrix(arr):
    NT = len(Eload)             # time step numbers
    Npv = np.around(arr[:,0])   # PV number
    Nwt = np.around(arr[:,1])   # WT number
    Nbat = np.around(arr[:,2])  # Battery pack number
    N_DG = np.around(arr[:,3])  # number of Diesel Generator
    Cn_I = arr[:,4]             # Inverter Capacity
...
def CostFunction(X,Eload, G, T, Vw):
